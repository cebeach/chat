...
Requires a running llama-server (llama-server) or Ollama instance (`ollama serve`).
...
Air-gapped terminal chat app talking to a local llama or Ollama server. Five source files, no package structure:
...
- **`llama_client.py`** -`LlamaClient` wraps the llama-server API. `LlamaChatStream` is an iterable that yields the chat response (SSE/OpenAI format) after iteration.
